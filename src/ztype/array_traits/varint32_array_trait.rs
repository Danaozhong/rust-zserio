...
impl array_trait::ArrayTrait<i32> for VarInt32ArrayTrait {
    ...
    fn bitsize_of(&self, _bit_position: u64, value: &i32) -> Result<u64> {
        ztype::varint32_bitsize(*value).map(|v| v as u64)
    }

    fn initialize_offsets(&self, bit_position: u64, value: &i32) -> Result<u64> {
        Ok(bit_position + self.bitsize_of(bit_position, value)?)
    }

    fn read(&self, reader: &mut BitReader, value: &mut i32, _index: usize) -> Result<()> {
    ...
    fn init_context(&self, context_node: &mut PackingContextNode, element: &i32) -> Result<()> {
