...
pub struct VarUintArrayTrait {}

impl array_trait::ArrayTrait<u64> for VarUintArrayTrait {
    ...
    fn bitsize_of(&self, _bit_position: u64, value: &u64) -> Result<u64> {
        ztype::varuint_bitsize(*value).map(|v| v as u64)
    }

    fn initialize_offsets(&self, bit_position: u64, value: &u64) -> Result<u64> {
        Ok(bit_position + self.bitsize_of(bit_position, value)?)
    }

    fn read(&self, reader: &mut BitReader, value: &mut u64, _index: usize) -> Result<()> {
        *value = ztype::read_varuint(reader)?;
        Ok(())
    }

    fn write(&self, writer: &mut BitWriter, value: &u64) -> Result<()> {
        ztype::write_varuint(writer, *value)
    }
    ...
    fn init_context(&self, context_node: &mut PackingContextNode, element: &u64) -> Result<()> {
