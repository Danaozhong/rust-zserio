...
pub fn write_float64(writer: &mut BitWriter, v: f64) -> Result<()> {
    write_bytes(writer, &f64::to_be_bytes(v).to_vec())
}

pub fn write_float32(writer: &mut BitWriter, v: f32) -> Result<()> {
    write_bytes(writer, &f32::to_be_bytes(v).to_vec())
}

pub fn write_float16(writer: &mut BitWriter, v: f32) -> Result<()> {
