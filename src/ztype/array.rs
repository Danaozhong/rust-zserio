...
impl<T> Array<T> {
	...
	pub fn marshal_zserio(
		...
		if self.is_packed {
			...
			for element in data {
				self.array_trait.init_context(self.packing_context_node.as_mut().unwrap(), &element);
			}
		}
		for (index, element) in data.iter().enumerate() {
			...
			if self.is_packed {
				self.array_trait.write_packed(self.packing_context_node.as_mut().unwrap(), writer, element);
			} else {
