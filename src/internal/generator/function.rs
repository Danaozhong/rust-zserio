...
use crate::internal::generator::expression::{
    generate_expression,
};
...
pub fn generate_function(codegen_scope: &mut Impl, zserio_function: &ZFunction) {
    ...
    let fn_gen_scope = codegen_scope.new_fn(convert_to_function_name(&zserio_function.name).as_str());
    ...
    fn_gen_scope.line(generate_expression(&zserio_function.result.as_ref().borrow()));
}
