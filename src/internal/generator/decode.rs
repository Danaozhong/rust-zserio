    ...
    if native_type.is_marshaler {
        ...
        if fund_type.bits != 0 {
            if fund_type.name == "int" {
                function.line(format!("{} = ztype::read_signed_bits(reader, {});", lvalue_field_name, fund_type.bits));
            } else {
                function.line(format!("{} = ztype::reader_unsigned_bits(reader, {});", lvalue_field_name, fund_type.bits));
            }
        } else if fund_type.name == "string" {
            // string types
            function.line(format!("{} = ztype::read_string(reader);", lvalue_field_name));
            ...
            function.line(format!("{} = ztype::read_{}(reader);", lvalue_field_name, rust_type_name));
        }
    }

    if field.is_optional {
        function.line(format!("{} = Option::from(optional_value);", rvalue_field_name));
